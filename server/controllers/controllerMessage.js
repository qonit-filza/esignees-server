const { Message, Notification, User, Document } = require('../models/index');
...
class Controller {
  ...
  static async sendMessage(req, res, next) {
    try {
      ...
      const { privateKeyString } = req.body;
      ...
      const status = await verifyPrivateKey(privateKeyString, sender.publicKey);

      console.log(status);

      //   let SenderEmail = req.user.name;
      //   let { email, message, privateKey } = req.body;
      ...
      //     UserId: user.id,
      //   });
      res.status(201).json({ valid: status });
    } catch (error) {
      next(error);
    }
  }
